...
do
    ...
    if not isAllowed then
        -- GitHub API config for key activation
        local GITHUB_TOKEN = "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" -- Replace with your GitHub PAT
        local KEYS_URL = "https://api.github.com/repos/Ygariqi/YMenu/contents/YMenu_Keys.txt"
        local WL_URL = "https://api.github.com/repos/Ygariqi/YMenu/contents/YMenu_Whitelist.txt"
        local API_HEADERS = {["Authorization"] = "token " .. GITHUB_TOKEN, ["Accept"] = "application/vnd.github.v3+json", ["Content-Type"] = "application/json"}
        ...
        -- Key input field
        local inputBg = Instance.new("Frame"); inputBg.Size = UDim2.new(1,-40,0,42); inputBg.Position = UDim2.new(0,20,0,165)
        inputBg.BackgroundColor3 = Color3.fromRGB(30,25,55); inputBg.BorderSizePixel = 0; inputBg.Parent = panel
        local inputCorner = Instance.new("UICorner"); inputCorner.CornerRadius = UDim.new(0,8); inputCorner.Parent = inputBg
        local inputStroke = Instance.new("UIStroke"); inputStroke.Color = Color3.fromRGB(80,60,140); inputStroke.Thickness = 1; inputStroke.Parent = inputBg
        
        local keyInput = Instance.new("TextBox"); keyInput.Size = UDim2.new(1,-20,1,0); keyInput.Position = UDim2.new(0,10,0,0)
        keyInput.BackgroundTransparency = 1; keyInput.Text = ""; keyInput.PlaceholderText = "YMENU-XXXXX-XXXXX-XXXXX"
        keyInput.TextColor3 = Color3.new(1,1,1); keyInput.PlaceholderColor3 = Color3.fromRGB(80,70,110)
        keyInput.TextSize = 15; keyInput.Font = Enum.Font.GothamBold; keyInput.ClearTextOnFocus = false; keyInput.Parent = inputBg
        
        -- Status label
        local statusLbl = Instance.new("TextLabel"); statusLbl.Size = UDim2.new(1,-40,0,20); statusLbl.Position = UDim2.new(0,20,0,215)
        statusLbl.BackgroundTransparency = 1; statusLbl.Text = ""; statusLbl.TextColor3 = Color3.fromRGB(130,80,255)
        statusLbl.TextSize = 12; statusLbl.Font = Enum.Font.GothamBold; statusLbl.Parent = panel
        
        -- Activate button
        local activateBtn = Instance.new("TextButton"); activateBtn.Size = UDim2.new(1,-40,0,40); activateBtn.Position = UDim2.new(0,20,0,242)
        ...
        activateBtn.MouseButton1Click:Connect(function()
            ...
            task.spawn(function()
                local ok, err = pcall(function()
                    -- 1. Fetch keys file
                    local keysRes = request({Url = KEYS_URL .. "?ref=main", Method = "GET", Headers = API_HEADERS})
                    if keysRes.StatusCode ~= 200 then error("Cannot fetch keys") end
                    local keysData = HttpService:JSONDecode(keysRes.Body)
                    local keysRaw = HttpService:JSONDecode('"' .. keysData.content:gsub("\n","") .. '"')
                    -- base64 decode
                    local decodedKeys = ""
                    pcall(function()
                        local b64 = keysData.content:gsub("%s+","")
                        local f = function(x) return string.char(tonumber(x,16)) end
                        -- Use syn/base64 decode or manual
                        local b='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
                        decodedKeys = b64:gsub('[^'..b..'=]','')
                        decodedKeys = decodedKeys:gsub('.',function(x)
                    ...
                    local keysSha = keysData.sha
                    
                    -- Check if key exists
                    local validKeys = {}
                    local keyFound = false
                    for line in decodedKeys:gmatch("[^\r\n]+") do
                        local trimmed = line:match("^%s*(.-)%s*$")
                        if trimmed and trimmed ~= "" then
                            if trimmed == key then
                                keyFound = true
                    ...
                    -- 2. Add username to whitelist
                    statusLbl.Text = "Adding username..."
                    local wlRes = request({Url = WL_URL .. "?ref=main", Method = "GET", Headers = API_HEADERS})
                    if wlRes.StatusCode ~= 200 then error("Cannot fetch whitelist") end
                    local wlData = HttpService:JSONDecode(wlRes.Body)
                    local wlSha = wlData.sha
                    
                    -- Decode whitelist
                    local decodedWl = ""
                    pcall(function()
                        local b64 = wlData.content:gsub("%s+","")
                        local b='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
                        decodedWl = b64:gsub('[^'..b..'=]','')
                        decodedWl = decodedWl:gsub('.',function(x)
                            if x=='=' then return '' end
                            local r,f='',(b:find(x)-1)
                            for i=6,1,-1 do r=r..(f%2^i-f%2^(i-1)>0 and '1' or '0') end
                            return r
                    ...
                    local wlNames = {}
                    for line in decodedWl:gmatch("[^\r\n]+") do
                        local trimmed = line:match("^%s*(.-)%s*$")
                        if trimmed and trimmed ~= "" then table.insert(wlNames, trimmed) end
                    end
                    table.insert(wlNames, player.Name)
                    
                    -- base64 encode new whitelist
                    local newWlContent = table.concat(wlNames, "\n") .. "\n"
                    local encodedWl = ""
                    pcall(function()
                        local b='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
                        encodedWl = ((newWlContent:gsub('.',function(x)
                            local r,bt='',x:byte()
                            for i=8,1,-1 do r=r..(bt%2^i-bt%2^(i-1)>0 and '1' or '0') end
                            return r
                        ...
                        end)..({'','==','='})[#newWlContent%3+1])
                    end)
                    
                    -- Update whitelist on GitHub
                    local updateWl = request({
                        Url = WL_URL, Method = "PUT", Headers = API_HEADERS,
                        Body = HttpService:JSONEncode({message = "Activate " .. player.Name, content = encodedWl, sha = wlSha, branch = "main"})
                    })
                    if updateWl.StatusCode ~= 200 and updateWl.StatusCode ~= 201 then error("Failed to update whitelist") end
                    
                    -- 3. Remove used key from keys file
                    statusLbl.Text = "Removing used key..."
                    local newKeysContent = table.concat(validKeys, "\n") .. "\n"
                    local encodedKeys = ""
                    pcall(function()
                        local b='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
                        encodedKeys = ((newKeysContent:gsub('.',function(x)
                            local r,bt='',x:byte()
                            for i=8,1,-1 do r=r..(bt%2^i-bt%2^(i-1)>0 and '1' or '0') end
                            return r
                        end)..'0000'):gsub('%d%d%d?%d?%d?%d?',function(x)
                            ...
                            for i=1,6 do c=c+(x:sub(i,i)=='1' and 2^(6-i) or 0) end
                            return b:sub(c+1,c+1)
                        end)..({'','==','='})[#newKeysContent%3+1])
                    end)
                    
                    request({
                        Url = KEYS_URL, Method = "PUT", Headers = API_HEADERS,
                        Body = HttpService:JSONEncode({message = "Used key by " .. player.Name, content = encodedKeys, sha = keysSha, branch = "main"})
                    })
                end)
                
                if ok then
                    statusLbl.Text = "✅ Activated! Reloading script..."; statusLbl.TextColor3 = Color3.fromRGB(50,200,100)
                    activateBtn.Text = "✅ SUCCESS"; activateBtn.BackgroundColor3 = Color3.fromRGB(30,150,70)
                    task.wait(2)
                    sg:Destroy()
                    -- Reload the script
                    loadstring(game:HttpGet("https://raw.githubusercontent.com/Ygariqi/YMenu/refs/heads/main/YMenu_V1.lua?t=" .. tostring(tick())))()
                else
